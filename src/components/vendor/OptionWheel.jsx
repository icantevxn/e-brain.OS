...
const OptionWheel = ({
  ...
  // LOCAL PATCH: the layout pass is split out of the rAF loop so it can also be
  // driven synchronously. Upstream only ever positioned options from inside
  // requestAnimationFrame, which leaves every option stacked at its untransformed
  // position whenever no frame runs — a hidden/background tab (rAF is suspended
  // there), or a render that re-attached the item refs after the last frame.
  ...
  // LOCAL PATCH: position the options before paint on every render. The rAF
  // loop still owns the easing between positions, but this guarantees the wheel
  // is laid out even when no frame ever runs, and re-applies styles to item
  // elements whose refs were just re-attached by a re-render.
  useLayoutEffect(() => {
  ...
  useEffect(
    () => () => {
      // LOCAL PATCH: clearing the handle matters as much as cancelling it.
      // startLoop() bails when rafRef.current is non-null, so leaving a stale
      // (already-cancelled) id here permanently wedges the loop — which is
      // exactly what StrictMode's mount/unmount/remount triggers in dev,
      // leaving every option stacked at its start position.
